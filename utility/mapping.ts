...
export interface KObjectMappingElement<TSource = any, TKType extends KTypeExtended = KTypeExtended> {
    $type?: TKType,
    $targetKey?: string,
    $convert?: (source: TSource) => TSource
    $convertBack?: (target: TSource) => TSource
    $fallbackValue?: TKType extends "kignore" ? TSource : never
}
...
function isKMapRecordReservedKey(key: string): boolean {
    return ["$type", "$targetKey", "$convert", "$convertBack", "fallbackValue"].includes(key)
}
...
export function mapBackKObject<T extends object>(data: KITEM2<T>, kMapRecord?: KObjectMappingRecord<T>): [T, KAttrRecord<T>] {
    if (kMapRecord == null) {
        if (data["@content"] || data["@attr"]) return [<any>data["@content"], <any>data["@attr"]]
        else return [<T>data, null]
    }
        ...
        do {
            ...
            if (targetValue["@attr"] != null) {
                ...
                if (targetAttr.__type != null) { // KITEM
                    ...
                    [result[k], resultAttr[k]] = <any>mapBackKObject<T[keyof T] & object>(targetResult, <any>targetMap)
                }
            } else { // KObject
                let targetResult = (targetMap.$convertBack != null) ? targetMap.$convertBack(<any>targetValue) : targetValue;
                [result[k], resultAttr[k]] = <any>mapBackKObject<T[keyof T] & object>(<any>targetResult, <any>targetMap)
            }
