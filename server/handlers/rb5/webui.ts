...
export namespace Rb5HandlersWebUI {
    export const updateSettings = async (data: {
        ...
        try {
            ...
            await DBH.update<IRb5PlayerBase>(data.refid, { collection: "rb.rb5.player.base" }, rb5Base)
            await DBH.update<IRb5PlayerConfig>(data.refid, { collection: "rb.rb5.player.config" }, rb5Config)
            await DBH.update<IRb5PlayerCustom>(data.refid, { collection: "rb.rb5.player.custom" }, rb5Custom)
            await DBH.upsert<IRb5Mylist>(data.refid, { collection: "rb.rb5.player.mylist" }, rb5Mylist)
            await DBH.upsert<IRbLobbySettings<5>>(null, { collection: "rb.rb5.player.lobbySettings#userId", userId: rb5Account.userId }, rb5LobbySettings)
            webUIMessage(send, "Save RB VOLZZA settings succeeded!", 5, WebUIMessageType.success)
        } catch (e) {
            webUIMessage(send, "Error occurred while saving RB VOLZZA settings: " + e.message, 5, WebUIMessageType.error)
        }
    }
    ...
    export const exportData = async (data: { refid: string }) => {
        ...
        return await DBH.overall(data.refid, account.userId, "rb.rb5", "export")
    }
}