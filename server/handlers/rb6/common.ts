...
import { findPlayerFromOtherVersion } from "../shared/find_player"
import { isToday } from "../../utils/utility_functions"
import { generateUserId } from "../shared/generate_user_id"
...
import { RbPlayerRead } from "../../models/shared/shared"
...
import { createReadCommentHandler, createWriteCommentHandler } from "../shared/comment"
import { createAddLobbyHandler, createReadLobbyHandler, createDeleteLobbyHandler } from "../shared/lobby"
...
export const bootPcb: H.H = () => XF.x(new Rb6ShopInfo())

export const readHitChartInfo: H.H = () => ({ ver: {} })

export const playerSucceeded: H.H = async data => {
...
export const startPlayer: H.H = async data => {
    const rid = $(data).str("rid")
    const account = await DB.FindOne<Rb6PlayerAccount>(rid, { collection: "rb.rb6.player.account" })
    ...
    result.quertCtrl.data = await Rb6Quest.createExamples(misc?.rankingQuestIndex ?? 0)
...
export const readPlayer: H.H<RbPlayerRead> = async data => {
...
export const deletePlayer: H.H = async data => {
    try {
        const rid = $(data).str("rid")
        const account = await DB.FindOne<Rb6PlayerAccount>(rid, { collection: "rb.rb6.player.account" })
...
export const writePlayer: H.H<Rb6Player> = async data => {
...
export const readPlayerScore: H.H = async data => {
    const rid = $(data).str("rid")
...
export const readPlayerJustCollections: H.H<Rb6ReadJustCollectionParameters> = async data => {
    const param = XF.o(data, Rb6ReadJustCollectionParameters)
    const result = new Rb6ReadJustCollection()
    result.justcollection.list.musicId = param.musicId
    result.justcollection.list.chartType = param.chartType
    if (param.userId === 1500008 && param.musicId === 1 && param.chartType === Rb6ChartType.hard) return XF.x(result)
...
export const readGhost: H.H<Rb6ReadGhostParam> = async data => {
...
async function writePlayerCore(player: Rb6Player) {
    ...
    if (player.pdata.stageLogs?.log?.length > 0) for (let i of player.pdata.stageLogs.log) await updateMusicRecordFromStageLog(rid, i, t, player.pdata.ghost?.list)
    if (player.pdata.justCollections?.list?.length > 0) for (let i of player.pdata.justCollections.list) await updateJustCollection(player.pdata.account.userId, i, t)
    if (player.pdata.characterCards?.list?.length > 0) for (let i of player.pdata.characterCards.list) t.upsert(rid, { collection: "rb.rb6.player.characterCard", characterCardId: i.characterCardId }, i)
    ...
    if (player.pdata.released?.info?.length > 0) for (const i of player.pdata.released.info) t.upsert(rid, { collection: "rb.rb6.player.releasedInfo", type: i.type, id: i.id }, i)
    if (player.pdata.playerParam?.item?.length > 0) for (const i of player.pdata.playerParam.item) t.upsert(rid, { collection: "rb.rb6.player.parameters", type: i.type, bank: i.bank }, i)
    ...
    if (player.pdata.ghost?.list?.length > 0) for (let g of player.pdata.ghost.list) await updateGhostScore(player.pdata.account.userId, g, t)
