...
import { getAvaliableMusicChartInfoFromMusicId, getMusicIdStr } from "../../data/musicinfo/rb_music_info"
import { IRb6CharacterCard } from "../../models/rb6/character_card"
import { IRb6ClasscheckRecord } from "../../models/rb6/classcheck"
...
export namespace Rb6HandlersWebUI {
    export const updateSettings = async (data: {
        ...
        try {
            ...
            rb6Base.name = data.name?.trim() ?? rb6Base.name
            ...
            await DBH.update<IRb6PlayerBase>(data.refid, { collection: "rb.rb6.player.base" }, rb6Base)
            await DBH.update<IRb6PlayerConfig>(data.refid, { collection: "rb.rb6.player.config" }, rb6Config)
            await DBH.update<IRb6PlayerCustom>(data.refid, { collection: "rb.rb6.player.custom" }, rb6Custom)
            await DBH.upsert<IRb6Mylist>(data.refid, { collection: "rb.rb6.player.mylist" }, rb6Mylist)
            await DBH.upsert<IRbLobbySettings<6>>(null, { collection: "rb.rb6.player.lobbySettings#userId", userId: rb6Account.userId }, rb6LobbySettings)
            await DBH.upsert<IRb6MiscSettings>(data.refid, { collection: "rb.rb6.player.misc" }, rb6MiscSettings)
            webUIMessage(send, "Save RB Reflesia settings succeeded!", 6, WebUIMessageType.success)
        } catch (e) {
            webUIMessage(send, "Error occurred while saving RB Reflesia settings: " + e.message, 6, WebUIMessageType.error)
        }
    }
    ...
    export const exportData = async (data: { refid: string }) => {
        ...
        return await DBH.overall(data.refid, account.userId, "rb.rb6", "export")
    }
    ...
    async function importAsphyxia(rb6Base: IRb6PlayerBase, profile: any, scores: any[], rid: string): Promise<IRb6PlayerBase> {
        try {
            ...
            for (let d of dataToUpsert) await DBH.upsert(rid, d.query, d.doc)
    ...
    async function queryRb6ScoreAt(rid: string, pageStr: string, pageSizeStr: string, sort: string, order: string, search: string, filter: string) {
        let musicRecords = await DB.Find<IRb6MusicRecord>(rid, { collection: "rb.rb6.playData.musicRecord" })
        let page = 0, pageSize = 0
        if (parseFloat(pageStr).toString() == pageStr) page = Math.trunc(parseFloat(pageStr))
        if (parseFloat(pageSizeStr).toString() == pageSizeStr) pageSize = Math.trunc(parseFloat(pageStr))
        let result: IRb6MusicRecord[] = []
        musicRecords = filterScores(musicRecords, filter)
        musicRecords = searchScores(musicRecords, search, 6)
    }
    let rangeGtRegex = /^((?<rangeRight>.+)(>(?<eqRight>=))[a-zA-Z_])*(>(?<eqLeft>=))(?<rangeLeft>.+)$/g
    let rangeLtRegex = /^((?<rangeLeft>.+)(<(?<eqleft>=))[a-zA-Z_])*(<(?<eqRight>=))(?<rangeRight>.+)$/g
    let rangeRegex = /^(?<rangeLeft>.+)[-~](?<rangeRight>.+)$/g
    let rangeBracketRegex = /^(?<eqLeft>[\(\[])(?<rangeLeft>.+),(?<rangeRight>.+)(?<eqRight>[\)\]])$/g
    let withinRegex = /(?<!\\)\|/
    function filterScores(records: IRb6MusicRecord[], filterStr: string): IRb6MusicRecord[] {
        if (!filterStr) return records
        let result: IRb6MusicRecord[] = []
        let filterParams = filterStr.split(/(?<!\\),/)
        for (let r of records) {
            let valid = true
            for (let fp of filterParams) {
                fp = fp.replace(/\\,/g, ",")
                let fpSplit = fp.split(/(?<!\\):/g)
                if (fpSplit.length != 2) continue
                let key = fpSplit[0].replace(/\\:/g, ":")
                let value = fpSplit[1].replace(/\\:/g, ":")
                // range
                let rangeLeft: number | undefined, rangeRight: number | undefined
                let eqLeftFlag = false, eqRightFlag = false
                let rangeMatch: RegExpExecArray
                if (rangeGtRegex.test(value)) rangeMatch = rangeGtRegex.exec(value)
                else if (rangeLtRegex.test(value)) rangeMatch = rangeLtRegex.exec(value)
                else if (rangeRegex.test(value)) {
                    rangeMatch = rangeRegex.exec(value)
                    eqLeftFlag = true
                    eqRightFlag = true
                } else if (rangeBracketRegex.test(value)) rangeMatch = rangeBracketRegex.exec(value)
                if (rangeMatch.length > 0) {
                    try {
                        rangeLeft = rangeMatch.groups.rangeLeft ? parseFloat(rangeMatch.groups.rangeLeft) : -Infinity
                        rangeRight = rangeMatch.groups.rangeRight ? parseFloat(rangeMatch.groups.rangeRight) : Infinity
                        if (rangeMatch.groups.eqLeft && rangeMatch.groups.eqLeft != "(") eqLeftFlag = true
                        if (rangeMatch.groups.eqRight && rangeMatch.groups.eqRight != ")") eqRightFlag = true
                        if (eqLeftFlag ? r[key] <= rangeLeft : r[key] < rangeLeft) valid = false
                        if (eqRightFlag ? r[key] >= rangeRight : r[key] > rangeRight) valid = false
                        continue
                    } catch { }
                }
                // collection
                if (withinRegex.test(value)) {
                    let withinValues = value.split(withinRegex)
                    for (let i = 0; i < withinValues.length; i++) withinValues[i] = withinValues[i].replace("\\|", "|")
                    let withinValuesSet = new Set(withinValues)
                    if (!withinValuesSet.has(r[key])) valid = false
                    continue
                }
                value = value.replace("\\|", "|")
                // neq
                if (value.startsWith("!=")) {
                    if (r[key] == value.substring(2)) valid = false
                    continue
                }
                if (value.startsWith("\\!=")) value = value.substring(1);
                // eq
                if (r[key] != value) valid = false
            }
            if (valid) result.push(r)
        }
        return result
    }
    function searchScores(musicScore: IRb6MusicRecord[], keywordStr: string, version: number) {
        let keywords = keywordStr.split(" ")
        let result: IRb6MusicRecord[] = []
        for (let m of musicScore) {
        }
        return result
    }
}