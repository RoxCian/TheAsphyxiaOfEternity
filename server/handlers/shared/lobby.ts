...
export function createAddLobbyHandler<TVersion extends RbVersion>(version: TVersion): H.H<RbLobbyEntry<TVersion>> {
    ...
    return async data => {
        ...
        await DBH.upsert(undefined, { userId: params.entries[0].userId, collection: `rb.rb${closure.version}.temporary.lobbyEntry` as const }, result.entries[0])
...
export function createDeleteLobbyHandler<TVersion extends RbVersion>(version: TVersion): H.H {
    ...
    return async data => {
        ...
        await DBH.remove<RbLobbyEntryElement<TVersion>>(undefined, { collection: `rb.rb${closure.version}.temporary.lobbyEntry` as const, entryId: entryId })
        return H.success
    }
}
