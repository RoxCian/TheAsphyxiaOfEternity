...
export namespace Rb3HandlersWebUI {
    export const updateSettings = async (data: {
        ...
        try {
            ...
            await DBH.update<IRb3PlayerBase>(data.refid, { collection: "rb.rb3.player.base" }, rb3Base)
            await DBH.update<IRb3PlayerConfig>(data.refid, { collection: "rb.rb3.player.config" }, rb3Config)
            await DBH.update<IRb3PlayerCustom>(data.refid, { collection: "rb.rb3.player.custom" }, rb3Custom)
            await DBH.upsert<IRb3Mylist>(data.refid, { collection: "rb.rb3.player.mylist" }, rb3Mylist)
            await DBH.upsert<IRbLobbySettings<3>>(null, { collection: "rb.rb3.player.lobbySettings#userId", userId: rb3Account.userId }, rb3LobbySettings)
            webUIMessage(send, "Save RB colette settings succeeded!", 3, WebUIMessageType.success)
        } catch (e) {
            webUIMessage(send, "Error occurred while saving RB colette settings: " + e.message, 3, WebUIMessageType.error)
        }
    }
    ...
    export const exportData = async (data: { refid: string }) => {
        ...
        return await DBH.overall(data.refid, account.userId, "rb.rb3", "export")
    }
}