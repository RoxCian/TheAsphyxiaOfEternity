...
import { convertToRb3ClearType, findAllBestMusicRecord } from "../shared/find_music_record"
...
export function registerRb3Handlers() {
    H.route("read.rb3info", readInfo)
    H.route("player.rb3start", startPlayer)
    H.route("player.rb3succeed", succeedPlayer)
    H.route("player.rb3read", readPlayer)
    H.route("player.rb3write", writePlayer)
    H.route("lobby.rb3entry", createAddLobbyHandler(3))
    H.route("lobby.rb3read", createReadLobbyHandler(3))
    H.route("lobby.rb3delete", createDeleteLobbyHandler(3))
    H.route("info.rb3pzlcmt_read", createReadCommentHandler(3))
    H.route("info.rb3pzlcmt_write", createWriteCommentHandler(3))
}
