...
import { StageLogManager } from "../shared/stage_log_manager"
...
import { RbPlayerRead, RbStageLogStandalone } from "../../models/shared/shared"
import { findPlayerFromOtherVersion } from "../shared/find_player"
import { convertToRb2ClearType, findAllBestMusicRecord } from "../shared/find_music_record"
...
export function registerRb2Handlers() {
    H.route("read.rb2info", readInfo)
    H.route("player.rb2start", startPlayer)
    H.route("player.rb2read", readPlayer)
    H.route("player.rb2write", writePlayer)
    H.route("log.rb2player", logPlayer)
    H.route("event_w.rb2update_status", updateEventStatus)
    H.route("lobby.rb2entry", createAddLobbyHandler(2))
    H.route("lobby.rb2read", createReadLobbyHandler(2))
    H.route("lobby.rb2delete", createDeleteLobbyHandler(2))
    H.route("info.rb2pzlcmt_read", createReadCommentHandler(2))
    H.route("info.rb2pzlcmt_write", createWriteCommentHandler(2))
}
