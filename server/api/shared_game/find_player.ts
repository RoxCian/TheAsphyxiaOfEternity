...
export async function findPlayerByUserIdFromOtherVersion(userId: number, forVersion?: RbVersion): Promise<FindPlayerResult> {
    ...
    // RB1
    if (forVersion !== 1) {
        base = await DBH.findOne<Rb1PlayerBase>(undefined, { collection: "rb.rb1.player.base", userId })
        if (base) {
            result.base = base
            result.name = base.name
            result.userId = base.userId
    ...
    // RB2
    if (forVersion !== 2) {
        base = await DBH.findOne<Rb2PlayerBase>(undefined, { collection: "rb.rb2.player.base", userId })
        if (base) {
            result.base = base
            result.name = base.name
            result.userId = base.userId
