...
const readPlayer: C.C<RbRequest, RbPlayerResponse> = async data => {
    ...
    const config = await DBH.findOne<Rb4PlayerConfig>(data.rid, { collection: "rb.rb4.player.config" }) ?? new Rb4PlayerConfig()
...
const rb4SettingsFactory: RbSettingsFactory<Rb4SettingsResponse, Rb4SettingsContext> = {
    contextQuery: {
        ...
        lobbySettings: contextQueryElement(ctx => ({ collection: "rb.rb4.player.lobbySettings#userId", userId: ctx.account.userId }), "public", ctx => new RbLobbySettings(version, ctx.account.userId)),
