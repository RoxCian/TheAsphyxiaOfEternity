...
const readQuestRecords: C.C<RbRequest, Rb6QuestRecordResponse[]> = async data => {
    ...
    for (const dungeon of dungeonsInfo) {
        ...
        const isUnlocked = records.find(r => r.dungeonId === dungeon.dungeonId) || dungeon.dungeonsIdToUnlock.length === 0 || dungeon.dungeonsIdToUnlock.every(i => records.find(r => r.dungeonId === i && r.isCleared))
...
const readRankingQuestRecords: C.C<RbRequest, Rb6QuestRecordResponse[]> = async data => {
    ...
    for (const q of rankingQuests) {
        const music0 = await tryFindMusicResponse(6, q.musicId0)
        const music1 = await tryFindMusicResponse(6, q.musicId1)
        const music2 = await tryFindMusicResponse(6, q.musicId2)
        if (!music0 || !music1 || !music2) continue 
        ...
        result.push({
            ...
            questType: Rb6QuestType.ranking,
            records: dungeonRecords
        })
    }
    return result
...
const readEquips: C.C<undefined, Rb6EquipmentInfo[]> = () => rb6Equips
const readCharacterCards: C.C<undefined, Rb6CharacterCardInfo[]> = () => rb6CharacterCards

const asphyxiaUploadSessionsContent: Record<string, { profileData: Buffer, profileProgress: number, scoresData: Buffer, scoresProgress: number }> = {}

const requestAsphyxia: C.C<RbRequest & { profileSize: number, scoresSize: number }, { profileProgress: number, scoresProgress: number }> = async data => {
    let sessionContent = asphyxiaUploadSessionsContent[data.rid]
    if (sessionContent && data.profileSize === sessionContent.profileData.length && data.scoresSize === sessionContent.scoresData.length) {
        return { profileProgress: sessionContent.profileProgress, scoresProgress: sessionContent.scoresProgress }
    }
    sessionContent = { profileData: Buffer.alloc(data.profileSize), profileProgress: 0, scoresData: Buffer.alloc(data.scoresSize), scoresProgress: 0 }
    asphyxiaUploadSessionsContent[data.rid] = sessionContent
    return { profileProgress: 0, scoresProgress: 0 }
}
const uploadAsphyxia: C.C<RbRequest & { file: "profile" | "scores", chunk: string }> = async data => {
    ...
    if (data.file === "profile") sessionContent.profileProgress = sessionProgress
    else sessionContent.scoresProgress = sessionProgress
    return `""`
}
...
const rb6SettingsFactory: RbSettingsFactory<Rb6SettingsResponse, Rb6SettingsContext> = {
    contextQuery: {
        ...
        lobbySettings: contextQueryElement(ctx => ({ collection: "rb.rb6.player.lobbySettings#userId", userId: ctx.account.userId }), "public", ctx => new RbLobbySettings(version, ctx.account.userId)),
...
async function toQuestRecordResponse(record: Rb6QuestRecord | undefined): Promise<Rb6QuestRecordResponseElement | undefined> {
